...
namespace KmsService.KeyBLL.CalendarStrategyHandler
{
    ...
    /// 库房外间申请职责链类
    /// </summary>
    public class OutsideWareHouseBLL:CalendarHandlerBLL
    {
        /// <summary>
        /// 库房外间申请职责链逻辑
        /// </summary>
        ...
        {
            string insideRoomName = ConfigurationManager.ConnectionStrings["outsideRoom"].ConnectionString;
            if (calendarModel.location.displayName.Contains(insideRoomName))
            {
                ...
                {
                    return insideRoomName + "+true";
                }
                else
                {
                    string content = string.Format("您申请的库房外间正在被{0}使用，请和{0}沟通好库房里间的使用时间。", calendarInfo.Organizer);
