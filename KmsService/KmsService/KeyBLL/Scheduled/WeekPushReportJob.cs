...
namespace KmsService.KeyBLL.Scheduled
{
    ...
    /// 每周推送报表创建任务
    /// </summary>
    public class WeekPushReportJob
    {
        /// <summary>
        /// 创建每周推送定时任务
        /// </summary>
        public void WeekPushPushTask()
        {
            //创建调度单元
            ...
            IJobDetail job = JobBuilder.Create<WeekPushReport>().WithIdentity("0 0 10 ? * MON").Build();
            //3.创建并配置一个触发器即trigger   1s执行一次
            ITrigger _CronTrigger = TriggerBuilder.Create()
              .WithIdentity("0 0 10 ? * MON")
              .WithCronSchedule("0 0 10 ? * MON") //秒 分 时 某一天 月 周 年(可选参数)
              .Build();
